...
class LtcSnapshotScriptTest(BitcoinTestFramework):
            ...
            fake_cli.write(textwrap.dedent("""\
                ...
                if role == "litecoin":
                    if command == "getblockchaininfo":
                        ...
                        if scenario.get("replace_snapshot_parent_with_directory_during_dump", False):
                            snapshot_parent = os.path.dirname(command_args[0])
                            os.rmdir(snapshot_parent)
                    ...
                    elif scenario.get("replace_snapshot_parent_with_directory_during_verify", False):
                        snapshot_parent = os.path.dirname(command_args[0])
                        snapshot_original_parent = snapshot_parent + ".original"
                        os.rename(snapshot_parent, snapshot_original_parent)
                        os.makedirs(snapshot_parent, exist_ok=True)
                    ...
                    if scenario.get("replace_audit_parent_with_directory_during_verify", False):
                        audit_path = os.environ["ZKCOIN_SNAPSHOT_AUDIT_JSON"]
                        audit_parent = os.path.dirname(audit_path)
                        os.rmdir(audit_parent)
    ...
    def run_snapshot(
        ...
        *,
        ...
        audit_path_symlink_parent_aliases_snapshot=False,
        audit_path_same_as_snapshot_incomplete=False,
        audit_path_aliases_snapshot_incomplete=False,
        audit_path_symlink_parent_aliases_snapshot_incomplete=False,
        ...
        audit_path_parent_replaced_during_verify=False,
        ...
        snapshot_path_parent_replaced_during_dump=False,
        snapshot_path_parent_replaced_during_verify=False,
        ...
        if snapshot_path_parent_missing:
            ...
            snapshot_path_parent_swapped_during_dump
            or snapshot_path_parent_swapped_during_verify
            or snapshot_path_parent_replaced_during_dump
            or snapshot_path_parent_replaced_during_verify
        ):
            snapshot_dir = os.path.join(self.options.tmpdir, f"{name}-snapshot-dir")
            os.makedirs(snapshot_dir, exist_ok=True)
            snapshot_path = os.path.join(snapshot_dir, f"{name}.dat")
        ...
        elif audit_path_symlink_parent_aliases_snapshot:
            alias_dir = os.path.join(self.options.tmpdir, f"{name}-snapshot-path-symlink-alias")
            os.symlink(self.options.tmpdir, alias_dir)
            audit_path = os.path.join(alias_dir, os.path.basename(snapshot_path))
        ...
        elif audit_path_symlink_parent_aliases_snapshot_incomplete:
            alias_dir = os.path.join(self.options.tmpdir, f"{name}-snapshot-incomplete-symlink-alias")
            os.symlink(self.options.tmpdir, alias_dir)
            audit_path = os.path.join(alias_dir, os.path.basename(snapshot_path) + ".incomplete")
        ...
        elif audit_path_parent_swapped_during_verify or audit_path_parent_replaced_during_verify:
            audit_dir = os.path.join(self.options.tmpdir, f"{name}-audit-dir")
            os.makedirs(audit_dir, exist_ok=True)
            audit_path = os.path.join(audit_dir, f"{name}.audit.json")
        ...
        if (
            ...
            or audit_path_symlink_parent_aliases_snapshot
            ...
            or audit_path_parent_replaced_during_verify
    ...
    def run_test(self):
        ...
        assert_equal(calls, [])
        _, calls, _ = self.assert_snapshot(
            "audit-path-symlink-parent-aliases-snapshot",
            self.scenario(),
            1,
            "snapshot audit summary path must differ from snapshot output path",
            audit_path_symlink_parent_aliases_snapshot=True,
        )
        assert_equal(calls, [])
        ...
        assert_equal(calls, [])
        _, calls, _ = self.assert_snapshot(
            "audit-path-symlink-parent-aliases-incomplete",
            self.scenario(),
            1,
            "snapshot audit summary path must differ from snapshot incomplete output path",
            audit_path_symlink_parent_aliases_snapshot_incomplete=True,
        )
        assert_equal(calls, [])
        ...
        self.log.info("Reject snapshot output directory replacement during dump")
        _, calls, _ = self.assert_snapshot(
            "snapshot-dir-replaced-during-dump",
            self.scenario(replace_snapshot_parent_with_directory_during_dump=True),
            1,
            "snapshot output directory changed during snapshot generation",
            snapshot_path_parent_replaced_during_dump=True,
        )
        assert not any(call["role"] == "zkcoin" for call in calls)
        ...
        self.log.info("Reject snapshot output directory replacement during verification")
        _, calls, snapshot_path = self.assert_snapshot(
            "snapshot-dir-replaced-during-verify",
            self.scenario(replace_snapshot_parent_with_directory_during_verify=True),
            1,
            "snapshot output directory changed during snapshot generation",
            snapshot_path_parent_replaced_during_verify=True,
        )
        self.assert_command(calls, "zkcoin", "verifysnapshotmanifest", [snapshot_path])
        ...
        self.log.info("Reject audit summary directory replacement during verification")
        _, calls, snapshot_path = self.assert_snapshot(
            "audit-dir-replaced-during-verify",
            self.scenario(replace_audit_parent_with_directory_during_verify=True),
            1,
            "snapshot audit summary directory changed during snapshot verification",
            audit_path_parent_replaced_during_verify=True,
        )
        self.assert_command(calls, "zkcoin", "verifysnapshotmanifest", [snapshot_path])
