...
class OrchardAuxPowRealProofTest(LocalLitecoinForkAuxPowTest):
    ...
    def run_test(self):
        ...
        self.log.info("Mine a real-proof spend from the full-reindexed cold peer after rollback")
        cold_peer.invalidateblock(spend_candidate["hash"])
        assert_equal(cold_peer.getblockcount(), 2)
        assert_equal(cold_peer.getbestblockhash(), shielded_candidate["hash"])
        assert real_spend_txid in cold_peer.getrawmempool()
        post_full_reindex_undo_info = cold_peer.getblockchaininfo()["shielded_pool"]
        assert_equal(Decimal(str(post_full_reindex_undo_info["value_pool"])), Decimal("1.00000000"))
        assert_equal(post_full_reindex_undo_info["commitments"], 1)
        assert_equal(post_full_reindex_undo_info["nullifiers"], 0)
        assert_equal(post_full_reindex_undo_info["anchors"], 2)
        assert_equal(cold_peer.gettxout(real_spend_txid, 0, False), None)
        restored_cold_peer_mint_output = cold_peer.gettxout(real_mint_txid, 0, False)
        assert_equal(Decimal(str(restored_cold_peer_mint_output["value"])), Decimal("3.99900000"))

        if self.is_wallet_compiled():
            post_full_reindex_spend_candidate = cold_peer.getauxblock()
        else:
            post_full_reindex_spend_candidate = cold_peer.createauxblock(cold_peer.get_deterministic_priv_key().address)
        assert_equal(post_full_reindex_spend_candidate["height"], 3)
        assert post_full_reindex_spend_candidate["hash"] != spend_candidate["hash"]
        post_full_reindex_parent_block = self.mine_parent_block(
            parent,
            commitment_hex=post_full_reindex_spend_candidate["auxpowcommitment"],
        )
        post_full_reindex_auxpow = build_parent_auxpow(post_full_reindex_parent_block)
        if self.is_wallet_compiled():
            assert_equal(
                cold_peer.getauxblock(
                    post_full_reindex_spend_candidate["hash"],
                    post_full_reindex_auxpow.serialize().hex(),
                ),
                True,
            )
        else:
            assert_equal(
                cold_peer.submitauxblock(
                    post_full_reindex_spend_candidate["hash"],
                    post_full_reindex_auxpow.serialize().hex(),
                ),
                True,
            )
        assert_equal(cold_peer.getblockcount(), 3)
        assert_equal(cold_peer.getbestblockhash(), post_full_reindex_spend_candidate["hash"])
        assert real_spend_txid in cold_peer.getblock(post_full_reindex_spend_candidate["hash"])["tx"]
        assert_equal(cold_peer.getrawmempool(), [])
        post_full_reindex_spend_info = cold_peer.getblockchaininfo()["shielded_pool"]
        assert_equal(Decimal(str(post_full_reindex_spend_info["value_pool"])), Decimal("0.00000000"))
        assert_equal(post_full_reindex_spend_info["commitments"], 1)
        assert_equal(post_full_reindex_spend_info["nullifiers"], 1)
        assert_equal(post_full_reindex_spend_info["anchors"], 2)
        assert_equal(cold_peer.gettxout(real_mint_txid, 0, False), None)
        post_full_reindex_spend_output = cold_peer.gettxout(real_spend_txid, 0, False)
        assert_equal(Decimal(str(post_full_reindex_spend_output["value"])), Decimal("4.99800000"))

        self.log.info("Restart full-reindexed cold peer after mining replacement real-proof spend")
        ...
        assert real_spend_txid in cold_peer.getblock(post_full_reindex_spend_candidate["hash"])["tx"]
        assert_equal(cold_peer.getrawmempool(), [])
        restarted_post_full_reindex_spend_info = cold_peer.getblockchaininfo()["shielded_pool"]
        assert_equal(Decimal(str(restarted_post_full_reindex_spend_info["value_pool"])), Decimal("0.00000000"))
        assert_equal(restarted_post_full_reindex_spend_info["commitments"], 1)
        assert_equal(restarted_post_full_reindex_spend_info["nullifiers"], 1)
        assert_equal(restarted_post_full_reindex_spend_info["anchors"], 2)
        assert_equal(self.root_hex_to_payload_bytes(restarted_post_full_reindex_spend_info["root"]), spend_vector["anchor"])
        assert_equal(cold_peer.gettxout(real_mint_txid, 0, False), None)
        restarted_post_full_reindex_spend_output = cold_peer.gettxout(real_spend_txid, 0, False)
        assert_equal(Decimal(str(restarted_post_full_reindex_spend_output["value"])), Decimal("4.99800000"))
        restarted_post_full_reindex_duplicate_output = cold_peer.gettxout(
            duplicate_proof_outpoint["txid"],
            duplicate_proof_outpoint["vout"],
            False,
        )
        assert_equal(Decimal(str(restarted_post_full_reindex_duplicate_output["value"])), Decimal("6.00000000"))

        self.log.info("Reconsider original real-proof chain after replacement restart")
        cold_peer.reconsiderblock(spend_candidate["hash"])
        self.connect_node_to_child(4)
        self.sync_blocks([child, cold_peer])
        ...
        self.log.info("Mine local continuation from restarted late peer after reindexed sync")
        restarted_late_peer_state = self.shielded_pool_snapshot(late_peer)
        if self.is_wallet_compiled():
            late_peer_continuation_candidate = late_peer.getauxblock()
        else:
            late_peer_continuation_candidate = late_peer.createauxblock(late_peer.get_deterministic_priv_key().address)
        assert_equal(late_peer_continuation_candidate["height"], 8)
        assert late_peer_continuation_candidate["hash"] != rejoined_duplicate_block_hash
        late_peer_continuation_parent_block = self.mine_parent_block(
            parent,
            commitment_hex=late_peer_continuation_candidate["auxpowcommitment"],
        )
        late_peer_continuation_auxpow = build_parent_auxpow(late_peer_continuation_parent_block)
        if self.is_wallet_compiled():
            assert_equal(
                late_peer.getauxblock(
                    late_peer_continuation_candidate["hash"],
                    late_peer_continuation_auxpow.serialize().hex(),
                ),
                True,
            )
        else:
            assert_equal(
                late_peer.submitauxblock(
                    late_peer_continuation_candidate["hash"],
                    late_peer_continuation_auxpow.serialize().hex(),
                ),
                True,
        ...
        self.log.info("Reindex late peer after mining local continuation from reindexed sync")
        self.restart_node(3, extra_args=self.child_launch_args(dump, verify) + ["-reindex-chainstate"])
        late_peer = self.nodes[3]
        self.assert_child_snapshot_imported(late_peer, dump, verify)
        self.assert_orchard_child_config(late_peer)
        assert_equal(late_peer.getbestblockhash(), late_peer_continuation_candidate["hash"])
        ...
        self.log.info("Restart late peer after full-reindex rollback recovery")
        self.restart_node(3, extra_args=self.child_launch_args(dump, verify))
        late_peer = self.nodes[3]
        self.assert_child_snapshot_imported(late_peer, dump, verify)
        self.assert_orchard_child_config(late_peer)
        ...
        self.log.info("Mine post-recovery continuation from restarted late peer")
        post_recovery_state = self.shielded_pool_snapshot(late_peer)
        assert_equal(post_recovery_state, restarted_late_peer_state)
        if self.is_wallet_compiled():
            post_recovery_candidate = late_peer.getauxblock()
        else:
            post_recovery_candidate = late_peer.createauxblock(late_peer.get_deterministic_priv_key().address)
        assert_equal(post_recovery_candidate["height"], 9)
        assert post_recovery_candidate["hash"] != late_peer_continuation_candidate["hash"]
        assert post_recovery_candidate["hash"] != rejoined_duplicate_block_hash
        post_recovery_parent_block = self.mine_parent_block(
            parent,
            commitment_hex=post_recovery_candidate["auxpowcommitment"],
        )
        post_recovery_auxpow = build_parent_auxpow(post_recovery_parent_block)
        if self.is_wallet_compiled():
            assert_equal(
                late_peer.getauxblock(
                    post_recovery_candidate["hash"],
                    post_recovery_auxpow.serialize().hex(),
                ),
                True,
            )
        else:
