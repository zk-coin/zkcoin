...
static const std::vector<unsigned char>& OrchardRealNativeProofPrefixV1()
{
    static const std::vector<unsigned char> prefix{
        'z', 'k', 'c', '-', 'o', 'r', 'c', 'h', 'a', 'r', 'd', '-', 'n', 'a', 't', 'i', 'v', 'e', '-', 'p', 'r', 'o', 'o', 'f', '-', 'v', '1'};
    return prefix;
}
...
[[maybe_unused]] static const std::vector<unsigned char>& OrchardRealVerifierInputPreimagePrefixV1()
{
    static const std::vector<unsigned char> prefix{
        'z', 'k', 'c', '-', 'o', 'r', 'c', 'h', 'a', 'r', 'd', '-', 'r', 'e', 'a', 'l', '-', 'i', 'n', 'p', 'u', 't', '-', 'v', '1'};
    return prefix;
}
...
bool DecodeOrchardNativeProofBytesV1(const std::vector<unsigned char>& proof_bytes, uint8_t proof_kind, const uint256& public_input_hash, std::vector<unsigned char>& native_proof_bytes)
{
    ...
    if (!std::equal(prefix.begin(), prefix.end(), proof_bytes.begin())) return false;
    ...
    data.insert(data.end(), verifier_key_hash.begin(), verifier_key_hash.end());
    const uint256 verifier_input_hash = Hash(data);
    if (!std::equal(verifier_input_hash.begin(), verifier_input_hash.end(), proof_bytes.begin() + verifier_input_hash_offset)) return false;

    uint32_t proof_len{0};
    for (size_t i = 0; i < sizeof(proof_len); ++i) {
        proof_len |= uint32_t{proof_bytes[proof_len_offset + i]} << (8 * i);
    }
    if (proof_len == 0 || proof_len != proof_bytes.size() - proof_offset) return false;

    native_proof_bytes.assign(proof_bytes.begin() + proof_offset, proof_bytes.end());
    return true;
}
...
bool OrchardRealVerifierInputHashV1(const std::vector<unsigned char>& proof, uint8_t proof_kind, const uint256& public_input_hash, uint256& verifier_input_hash)
{
    ...
    const int ok = zkc_shielded_orchard_real_verifier_input_hash_v1(
        proof.data(),
        proof.size(),
        proof_kind,
        public_input_hash.begin(),
...
extern "C" int zkc_shielded_check_bundle_v5(
    const unsigned char* bundle,
    size_t bundle_len,
    uint8_t proof_kind,
    const unsigned char* public_input_hash,
    ...
    *proof_body_mode_out = Consensus::ShieldedPool::SHIELDED_ORCHARD_PROOF_BODY_MODE_UNKNOWN;
    ...
    if (proof_payload.size() < real_proof_offset ||
        zkc_shielded_orchard_real_verifier_input_hash_v1(
            ...
            real_verifier_input_hash_out_len) != 1) {
        std::fill(real_verifier_input_hash_out, real_verifier_input_hash_out + real_verifier_input_hash_out_len, 0);
        return Consensus::ShieldedPool::SHIELDED_ORCHARD_REAL_PROOF_STATUS_MALFORMED;
    }
    return status;
    ...
    size_t public_input_hash_len)
{
    ...
    return Consensus::ShieldedPool::DecodeOrchardNativeProofBytesV1(proof_bytes, proof_kind, public_input_hash_value, native_proof_bytes)
...
extern "C" int zkc_shielded_orchard_real_verifier_input_hash_v1(
    const unsigned char* proof,
    size_t proof_len,
    uint8_t proof_kind,
    const unsigned char* public_input_hash,
...
{
    if (verifier_input_hash_out == nullptr) return 0;
    if (verifier_input_hash_out_len != Consensus::ShieldedPool::SHIELDED_PROOF_HASH_SIZE) return 0;
    ...
    const uint256 verifier_key_hash = Consensus::ShieldedPool::ExpectedOrchardRealVerifierKeyHashV1();
    std::vector<unsigned char> data = Consensus::ShieldedPool::OrchardRealVerifierInputPreimagePrefixV1();
    data.push_back(proof_kind);
    data.insert(data.end(), public_input_hash_value.begin(), public_input_hash_value.end());
    data.insert(data.end(), verifier_key_hash.begin(), verifier_key_hash.end());

    const uint256 verifier_input_hash = Hash(data);
    std::copy(verifier_input_hash.begin(), verifier_input_hash.end(), verifier_input_hash_out);
    return 1;
}
...
extern "C" int zkc_shielded_orchard_real_proof_check_v2(
    const unsigned char* proof,
    size_t proof_len,
    uint8_t proof_kind,
    const unsigned char* public_input_hash,
