...
The read-only check also reports the remaining blocker count and next blocker
that would remain after applying the audit, so reviewers can confirm the
handoff advances the expected profile before any manifest write.
