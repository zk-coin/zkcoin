...
- `contrib/devtools/zkcoin_public_launch_profile_manifest.json` is the
  ...
  `contrib/devtools/zkcoin_public_launch_profile.py --network-value-selection-later-blockers NETWORK`
  ...
  `contrib/devtools/zkcoin_public_launch_profile.py --value-selection-candidate-artifact-status`
  ...
  Use
  `contrib/devtools/zkcoin_public_launch_profile.py --release-evidence-publication-closeout-checklist <release_evidence_publication_index_archive_record.json> <release_evidence_publication_index.json> <release_evidence_archive_record.json> <release_evidence_bundle.json>`
  after the publication index archive record verifies when release dashboards
  need one final read-only checklist that ties together the retained bundle,
  archive record, handoff summary JSON, publication index, publication index
  archive record, and publication index archive handoff summary JSON without
  choosing publication URIs, timestamps, publishers, or release commits.
  Use
  `contrib/devtools/zkcoin_public_launch_profile.py --check-release-evidence-publication-closeout <release_evidence_publication_closeout.json> <release_evidence_publication_index_archive_record.json> <release_evidence_publication_index.json> <release_evidence_archive_record.json> <release_evidence_bundle.json>`
  after retaining that closeout JSON to verify it still matches the current
  closeout checklist. Add `--require-release-evidence-publication-closeout-match`
  when CI should fail on closeout drift while still emitting the same JSON
  payload.
  Use
  `contrib/devtools/zkcoin_public_launch_profile.py --check-release-evidence-publication-closeout-archive <release_evidence_publication_closeout_archive_record.json> <release_evidence_publication_closeout.json> <release_evidence_publication_index_archive_record.json> <release_evidence_publication_index.json> <release_evidence_archive_record.json> <release_evidence_bundle.json>`
  after operators archive the retained closeout JSON to verify the archive
  record fields, closeout digest, closeout schema version, and current closeout
  gate output. Add
  `--require-release-evidence-publication-closeout-archive-match` when CI
  ...
  `command_keys`, `command_key_count`, `command_values`,
  `command_value_count`, `command_pairs`, `command_pair_count`, and
  `command_count`; the map groups the same read-only action-plan, next-action,
  readiness-summary, and status-json entry points, the key list exposes the map
  iteration order, the value list exposes the ordered command strings, the
  ...
  `later_command_value_counts`, `later_command_pairs`, and
  `later_command_pair_counts` mirror the same queue as ordered command maps,
  compact non-null command values, explicit key/value pairs, and their counts.
  ...
  blocker summary without scraping text.
  ...
  `external_artifacts_by_network_and_blocker_type`,
  `external_artifact_counts_by_network_and_blocker_type`,
  `snapshot_audit_external_artifacts_by_network`, and
  `snapshot_audit_external_artifact_counts_by_network` similarly expose the
  ...
  global blocker-group list. `blocked_field_groups_by_blocker` and
  `blocked_field_group_counts_by_blocker` expose the same group queue keyed by
  blocker id for direct lookup. Each blocked field group also includes
  ...
  `next_blocked_fields_by_network_and_blocker_type`, and
  ...
  `next_command_pair_count` exposes the pair list length directly, while
...
contrib/devtools/zkcoin_public_launch_profile.py \
  --release-evidence-publication-closeout-checklist <release_evidence_publication_index_archive_record.json> \
  <release_evidence_publication_index.json> \
  <release_evidence_archive_record.json> \
  <release_evidence_bundle.json> \
  contrib/devtools/zkcoin_public_launch_profile_manifest.json
...
contrib/devtools/zkcoin_public_launch_profile.py \
  --check-release-evidence-publication-closeout-archive <release_evidence_publication_closeout_archive_record.json> \
...
Add `--json` to the snapshot audit handoff command when automation needs
...
Add `--json` to the snapshot audit template-diff command when automation needs
machine-readable parse/object status, missing fields, unexpected fields, field
order compatibility, source-chain matching, and follow-up commands for an
operator-supplied audit summary without verifying the snapshot artifact or
accepting any constants.
...
value-selection blockers, blocked fields, JSON-capable candidate check command
templates, ordered pre-apply candidate checklists, and per-blocker readiness
summary commands without parsing the human handoff text.
...
Add `--json` to the blocker-type readiness-summary command when automation
needs machine-readable current blocker state for one blocker type, blocked
fields, blocked and ready networks, candidate constraints, and command
shortcuts.
Add `--json` to the readiness-gate summary command when automation needs
machine-readable current blocker state for one readiness gate, blocked fields,
blocker field groups, candidate constraints, blocker-type commands, and command
shortcuts.
...
Add `--json` to the readiness-gate later-blocker command when automation needs
machine-readable all remaining blockers for one readiness gate, blocked
fields, blocker field groups, candidate constraints, blocker-type commands, and
readiness-summary command map.
...
Use `--release-evidence-publication-closeout-checklist --json` after that
handoff summary verifies when automation needs a single closeout artifact set
covering the release evidence bundle, archive record, release evidence handoff
...
to verify a retained closeout JSON against the current closeout checklist. Add
`--require-release-evidence-publication-closeout-match` when CI should reject
closeout drift; status JSON exposes
`check_release_evidence_publication_closeout_command` and
`release_evidence_publication_closeout_gate_command` so dashboards can discover
the verifier and gate without hard-coding command strings.
Use `--check-release-evidence-publication-closeout-archive --json` after the
retained closeout JSON is archived. The archive record must include
`release_evidence_publication_closeout_uri`,
`release_evidence_publication_closeout_sha256`,
`release_evidence_publication_closeout_schema_version`, `manifest_path`,
`manifest_commit`, `gate_command`, `gate_verified`, `gate_mismatch_count`, and
`gate_checked_at`; the verifier compares the deterministic fields to the
current closeout gate and only requires operator-supplied URI, commit, and
